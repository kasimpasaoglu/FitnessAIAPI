...
public class UserValidator : AbstractValidator<UserVM>
{
    ...
    public UserValidator()
    {
        ...
        RuleFor(x => x.Goal)
            ...
            .Must(g => FitnessGoals.Goals.Contains(g)).WithMessage($"Goal must be one of the following: {string.Join(", ", FitnessGoals.Goals)}.");

        RuleFor(x => x.AvailableDays)
            .NotEmpty().WithMessage("Available days are required.");
        RuleForEach(x => x.AvailableDays)
            .Must(FitnessGoals.Days.Contains)
            .WithMessage($"Each day must be one of the following: {string.Join(", ", FitnessGoals.Days)}.");
